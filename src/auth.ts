...
const fetchSpotifyAccessToken = async (
  ...
  if (useCache) {
    const cached = caches.spotify.accessToken.get<OAuthAccessToken>(
      refreshToken
    );
    if (cached) {
  ...
  if (useCache) {
    ...
    caches.spotify.accessToken.set<OAuthAccessToken>(
      refreshToken,
      accessToken,
      ttl
    );
    ...
    async (req: Request, done) => {
      ...
      if (!user.spotifyRefreshToken && user.sub) {
        ...
        if (cached) {
          ...
          logger.debug("Using cached Spotify refresh token", {
            user_id: userId
          });
        } else {
          try {
            ...
            if (auth0User.identities) {
              ...
              if (identity) {
                ...
                caches.spotify.refreshToken.set(
                  user.sub,
                  user.spotifyRefreshToken
                );
              } else {
                logger.warn("No Spotify identity found for Auth0 user", {
                  user_id: userId
                });
              }
            }
          } catch (error) {
            logger.error("Error occurred fetching the Auth0 user details", {
              user_id: userId,
              error
            });
            done(error, false);
        ...
        try {
          ...
          const accessToken: OAuthAccessToken | undefined =
            ...
            user.spotifyAccessTokenExpiresAt <
              now - ACCESS_TOKEN_SLACK_SECONDS * 1000
              ? {
                  token: user.spotifyAccessToken,
                  expiresAt: user.spotifyAccessTokenExpiresAt
                }
          ...
          console.error(`An error occurred fetching Spotify access token`, {
            userId,
            error
          });
          done(error, false);
